...
class MosaicCellHolder(Task):
	"""Make image mosaic of the cell holder."""
	...
	def worker(self):
		...
		for cell in self.cm.cellList:
			if cell.slot.slot_holder.identiy == 'cell_holder':
		...
		mosaic.save(folder+"holder "+time.strftime('%Y-%m-%d %H%M')+'.jpg', "JPEG")
